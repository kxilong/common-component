...
export default {
  ...
  mounted() {
    // 计算列表如果全部渲染应该有的高度
    this.$refs.scrollHeight.style.height = this.items.length * this.size + "px";
    // 计算可视区域的高度
    this.$refs.wrap.style.height = this.remain * this.size + "px";
    // 缓存每一项的高度
  ...
  updated() {
    this.$nextTick(() => {
      ...
      domArr.forEach(item => {
        ...
        if (difference) {
          // 如果高度变了，更新这一项的高度和 bottom（height 的变化不影响 top）
          this.positionListArr[id].height = height;
          this.positionListArr[id].bottom =
            this.positionListArr[id].bottom - difference;
          // 后面的每一项相应的也要调整
          for (let i = id + 1; i < this.positionListArr.length; i++) {
            if (this.positionListArr[i]) {
              ...
              this.positionListArr[i].bottom =
                this.positionListArr[i].bottom - difference;
            }
          }
        }
      });
      ...
      this.$refs.scrollHeight.style.height =
        this.positionListArr[this.positionListArr.length - 1].bottom + "px";
    });
  },
  methods: {
    back() {
    ...
    handleScroll() {
      ...
      if (this.variable) {
        ...
        this.offset = this.positionListArr[this.start - this.prevCount]
          ? this.positionListArr[this.start - this.prevCount].top
          : 0;
      } else {
        // 每一项高度固定
    ...
    cacheListPosition() {
      ...
      // 注意: () => ({}) 为返回对象的写法
      this.positionListArr = this.items.map((item, index) => ({
        height: this.size,
        top: index * this.size,
